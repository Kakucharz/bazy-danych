...
--ZAD 6k
SELECT pen.stanowisko, COUNT(pra.id_pracownika) AS n_pracownikow
FROM ksiegowosc2.pensja pen
JOIN ksiegowosc2.wynagrodzenia2 w ON w.id_pensji = pen.id_pensji
JOIN ksiegowosc2.pracownicy pra ON w.id_pracownika = pra.id_pracownika
GROUP BY pen.stanowisko;

--ZAD 6l
SELECT MIN(kwota) AS min, MAX(kwota) AS maks, AVG(kwota) AS œrednia
FROM ksiegowosc2.pensja
WHERE stanowisko LIKE '%Kierownik%';

--ZAD 6m
SELECT SUM(pen.kwota) AS kwota
FROM ksiegowosc2.wynagrodzenia2 w
JOIN ksiegowosc2.pensja pen ON w.id_pensji = pen.id_pensji;
...
--ZAD 6n (f)
SELECT pen.stanowisko, SUM(pen.kwota) AS kwota
FROM ksiegowosc2.wynagrodzenia2 w
JOIN ksiegowosc2.pensja pen ON w.id_pensji = pen.id_pensji
GROUP BY pen.stanowisko;

SELECT stanowisko, SUM(kwota) AS kwota
FROM ksiegowosc2.pensja
GROUP BY stanowisko;

--ZAD 6o (g)
SELECT pen.stanowisko, COUNT(w.id_premii) AS n_premii
FROM ksiegowosc2.wynagrodzenia2 w
JOIN ksiegowosc2.pensja pen ON w.id_pensji = pen.id_pensji
GROUP BY pen.stanowisko;

--ZAD 6p (h)
DELETE FROM ksiegowosc2.pracownicy
